	...
	<div class="top2">
		...
		<div class="fr top-link">
			...
			<span id="admin" @click="choseItem('adminList')"><i
				class="adminIcon"></i><span v-text="adminusername"></span></span> 
		</div>
	</div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getMenu() {
                this.adminMenu = window.JSON.parse(window.localStorage.getItem('menu')) || [];
                var login = this.$store.getters.getLoginData;
                if (!this.adminMenu[0]) {
                    window.location.href = "/#/admin/login";
                }
                this.adminusername = this.adminMenu.join(',');
